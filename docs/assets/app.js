...
(function () {
  ...
  //   五轮按用户反馈简化：去掉点击开合 / 药丸样式 / 自动收起，纯 hover 展开）──
  //   常驻 = 右缘一个半透明圆形按钮（纯 hover 触发，非交互元素）；指针移到
  //   按钮区域展开模块名列表（当前所在模块的线更长、主题色，IntersectionObserver
  //   判定）；点击条目跳转对应 section（scroll-margin-top 已让过页头 + 吸顶条），
  //   并 history.replaceState 同步 section 深链参数（不触发 route 重渲染）。
  //   仅画布视图显示（route() 控制 hidden），窄屏隐藏。
  function buildCanvasNav() {
    ...
    nav.innerHTML = CANVAS_SECTIONS.map(function (s) {
      ...
      '<span class="canvas-nav-fab" aria-hidden="true">☰</span>';
  }
