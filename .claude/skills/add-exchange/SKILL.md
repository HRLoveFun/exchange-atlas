...
**新增一家交易所默认是按需可选能力（[ADR-041]）：只在用户主动要求时执行，agent 不主动提议或启动，也不在下一步任务建议里列「加某家交易所」。** 例外：当 ROADMAP §三「广度扩张 · ① skill 迭代轨」活跃时，用实际新增来迭代本 skill 是一条已排期的开发轨（[ADR-add-exchange-skill-forge]，当前目标 = 越南市场），按该条目推进即可。补全已有交易所的空字段不受此限。
...
- **多个子代理各自独立为同一个英文概念新造中文译法、互不知情，是新观察到的一种重名冲突。**
  ...
  不要想当然自己是第一个遇到这个概念的人。**
- **要写 ADR（发现 schema 设计问题、定了个会影响后续的取舍）时**，在 `DECISIONS.md` 追加一条
  `### ADR-<slug>`（kebab，取自分支名 / 主题；不再用数字号）。并行分支各加了同名 slug → `DECISIONS.md`
  上的可见文本冲突，串行合并时暴露、按 [ADR-029] 让 slug + grep 改引用。见 [ADR-slim-coordination-machinery]。
