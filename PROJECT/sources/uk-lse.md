...
- `docs.londonstockexchange.com`（2026-09-05 补充登记，任务四收尾第29条，沿用既有域名条目）
  ...
  - 2026-09-05 二次补充（任务四微缺口收口）: Service Announcement Notice 001/05122023 — London Stock Exchange – 05 December 2023 - Technical Incident Update（官方事后公告：撮合引擎分区3两次盘中暂停的根因/触发与复牌时刻逐字记载，并回溯 2023-10-19 RFQ/ETP 软件问题——major_outage_history 的 2023-12 事件出处）: https://docs.londonstockexchange.com/sites/default/files/documents/live_-_001_-_05122023_-_london_stock_exchange_-_05_december_2023_-_technical_incident_update.pdf（HTTP 200，118KB）
- `api.londonstockexchange.com`（2026-09-05 新增登记，任务四微缺口收口） | 官方（主站官方内容 API——SPA 页运行时即从该 API 取正文，等于「Business days 页本体」的可 curl 形态） | en | curl 常规 UA 200，返回 HTTP JSON；⚠️ 页面正文以 HTML 片段内嵌在 JSON 字符串里，`&amp;`/`&nbsp;` 为原样实体、换行为 JSON `\n` 转义，quote 须照抄实体；发现方式：Chrome headless `--log-net-log` 抓主站 SPA 页的 XHR（主站 `www.londonstockexchange.com` 仍是 SPA 壳，见上方既有条目，但其动态数据层可 curl——SPA 站点遇同类缺口可先试这个办法） | 用于 holidays_note（政策句 + 2026-2029 逐日银行假日表）
  - Business days 页内容 API（与主站 /securities-trading/trading-access/business-days 对应）: https://api.londonstockexchange.com/api/v1/pages?path=equities-trading/business-days&parameters=（HTTP 200，42KB）
...
- `data.iana.org`（2026-09-05 新增登记，stable 档来源闭环） | 官方（IANA 时区数据库，时区命名的权威登记处，跨所通用——各交易所 `overview.timezone`/`overview.dst_rule` 出处） | en | curl 常规 UA 200，静态纯文本，zone 文件随 tzdata 版本更新 | Europe/London（GMT/BST 及夏令时切换 Rule） 的 UTC 偏移与夏令时规则
  - tzdb「europe」zone 文件（`Europe/London`）: https://data.iana.org/time-zones/tzdb/europe（HTTP 200，64KB）
