...
- **szse.cn 的可达性按子域分裂（2026-09-06 住宅 IP 实测修正）：www.szse.cn / english.szse.cn 主站 TCP 层即不通（海外 IP 被拒，HTTP 000），但 `docs.static.szse.cn`（全部规则 PDF 的静态 CDN）与 `investor.szse.cn`（投资者教育子站，含官方英译规则 PDF）均 200 可达。** 结论：深交所规则原文（中文全文 + 创业板英译）实际上**拿得到**，「官网不可达」的记录必须先分子域再下。wayback 对 english.szse.cn 及其 2024 英文主板规则 PDF（`www.szse.cn/English/rules/siteRule/P020240911599025860114.pdf`，仅挂主站）确无快照——「官方 PDF 已定位 URL」≠「拿得到」，wayback 覆盖中国交易所官网极稀疏，别指望回退。另注意：SZSE 官方中英文一手对退市整理期 15 交易日均用拼写数字（中文「十五个交易日」、创业板英译「fifteen trading days」），同份英译对复核期却混用阿拉伯「15-trading day period」——**官方翻译数字写法不统一，找阿拉伯数字锚点要逐条核对语义，不能只看「有 15」**。
