...
| `financial_transaction_tax` | C | **DOWNGRADE** | 来源仅 ASX 费率表，无联邦/州税制正面陈述；→ `rate: null` |
...
| `financial_transaction_tax` | A | **FIX** | `rate: 0` 语义正确（IOF 税种存在、现行 0%，与 `type: none` 不同）；补 `side: buy`（quote "incoming resources" = 买入侧） |
...
| `financial_transaction_tax` | C | **DOWNGRADE** | detail 自述「『未见』不等于穷尽性排除」→ `rate: null` |
...
| `stamp_duty` | C | **DOWNGRADE** | quote 只讲 2026 STT 上调，无「无印花税税种」正面陈述 → `rate: null` |
| `financial_transaction_tax` | B | PASS | 0.2 有 zh 支撑。⚠️ `side: sell` 无出处 → note 补强 + OPEN-Q |
...
| `stamp_duty` | A | PASS | 0.5 ⊆ quote。⚠️ `side: buy`（SDRT 向买方课征）quote/zh 均未明说 → 保留值（渲染层缺省回退 `both`，移除错得更远）+ OPEN-Q 补强 |
...
| `regulatory_fees` | A | PASS | 20.60/0.00 ⊆ quote。⚠️ 本字段 quote 写 "securities transactions" 未提 "covered sales" → OPEN-Q 补强 |
...
| `stamp_duty` | D | PASS | `rate: null` 恰当（quote 为逗号小数 "0,25%"）。⚠️ 25 bp 的最大成本项进不了 spec（渲染幽灵条）+ `side: buy` 无出处，见 OPEN-Q |
