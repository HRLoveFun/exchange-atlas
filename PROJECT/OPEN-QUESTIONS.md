...
3. **受控词表（enums.yml）够不够用** — 五家标杆的机制差异能否被现有枚举值（如 `price_limit_type`: none/percentage_band/absolute_tiered/dynamic_reference）概括，还是每家都要新增例外值。如果某个字段的例外多到枚举失去意义，结论应该是这个字段不适合进矩阵（改 `in_matrix: false`），而不是无限加枚举值。**`review_system` 已经积累了五个互不相同的真实案例**：上交所"注册制"（`registration`枚举）、港交所"披露为本但上市委员会有实质审核权"（未套用枚举，直接留 zh 文字说明）、NYSE"披露为本+非常具体的量化规则编号（如 Rule 102.01C(I)）+ 广泛自由裁量权"（同样未套枚举）、JPX"披露为本，交易所审核+重大事项另须向金融厅报告"（同样未套枚举）、ASX"披露为本，满足量化门槛（利润/资产测试二选一）+股东分布/自由流通量要求即可申请，交易所对适合性另有实质审核权"（同样未套枚举，v1.0首次压测样本）——五家没有一家能被现有 `review_system` 枚举值干净覆盖，这个字段目前实质上已经退化成自由文本描述而非可比较的枚举，值得考虑要么扩充枚举维度（如拆成"审核严格度"+"规则形式化程度"两个正交维度），要么承认这个字段不适合进矩阵横向比较，只适合放在档案页。
18. **`listing.boards` 假设的"多层级板块体系"在 `au-asx` 不成立，是继 `de-eurex`（衍生品所无"公司上市"概念，见第17条）之后第二种 listing 章节结构性不完全适配的样本，但程度更轻。** ASX不像SSE（主板/科创板/创业板）、HKEX（主板/GEM）、JPX（Prime/Standard/Growth）那样有分层板块，只有单一准入名单（利润测试或资产测试二选一 + 股东分布/自由流通量门槛）+ "ASX Foreign Exempt Listing"（面向已在合格境外交易所上市公司的另一准入路径，本质是豁免类别不是板块）。`au-asx.yml` 的 `listing.boards` 因此填了两条"伪板块"（`au-asx-official-list`/`au-asx-foreign-exempt`）勉强塞进 list 结构，`transfer_between_boards` 字段填"不适用"。与Eurex案例（整章留空）不同，这里章节本身是适用的（ASX确实有"公司上市"概念），只是"多层级板块"这个子假设不适用——说明 listing 章节内部至少存在"整章不适用"（衍生品所）与"boards 多层级假设不适用但章节其余部分适用"（单层准入名单的现货交易所）两种不同程度的不适配，`taxonomy.yml` 目前未区分处理，继续沿用"如实留空+detail说明"的方式吸收，暂不改schema。
19. **`legislation.gov.au` 的 `/latest/text` 端点只返回法律的章节目录索引，不含逐条正文**（约6800行全是标题树，无正文段落）——首次踩坑记录见 `SOURCES.md` `au-asx` 一节该域名条目备注。这是继 sec.gov/finra.org/dtcc.com（403反爬，见第14/15条）之后第二类"域名能抓到但内容不是想要的东西"的坑，与反爬无关、纯粹是该网站信息架构问题。下次遇到类似的联邦/国家法律数据库网站，先用一小段探测确认该URL返回的是正文还是目录索引，避免误以为"200+大文件=拿到全文"。
