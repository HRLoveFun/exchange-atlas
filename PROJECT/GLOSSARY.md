...
| 违约损失分摊阶梯 | Default Waterfall | en: Default Waterfall | CCP处置结算参与者违约损失时依次动用各类资源的先后顺序——典型排序为：违约方自身保证金/结算基金出资在先，其次是CCP自有资本注资（Skin in the Game），最后才动用非违约参与者按比例分摊的结算基金出资；港交所HKCC按其规则第706条的8步排序是本项目首次完整记录的样本（见 hk-hkex.yml clearing.default_management），日后遇到其他交易所披露类似"default waterfall"文件时统一用此译法比较各所排序细节异同 |
