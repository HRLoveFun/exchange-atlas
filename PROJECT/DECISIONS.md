...
### ADR-phase4-canvas-polish — Phase 4 棒 5：跨模块视觉语言对账 + 已知视觉遗留收口 + 20 家巡检 + 非专业读者实测

**背景：** [ADR-phase4-canvas-layout] 棒 2–4 落地后画布外壳已定型，棒 5（本条）是 Phase 4 的最后一棒：headless 全量巡检、[ADR-040] 线条语言表跨整块画布重新校、穿插 §一 已知视觉遗留、收口 gate（「非专业读者滚一遍画布看懂一个市场」实测，Phase 2 收口 gate 的等价关卡）。

**跨模块视觉语言对账（实测结论）：**

- **令牌纪律**：`app.js` 十六进制色字面量 0 个（grep 全扫），7 个模块全部走 `styles.css` 令牌——跨模块统一由令牌层结构性保证，不靠各模块自觉。
- **色彩语义表（跨 7 模块统一，此前散在各设计 ADR，此处汇总为整块画布的一张表）**：红 `--danger` = 约束与风险（涨跌停墙 / 熔断触发线〔[ADR-040] 表〕/ 上市生命周期退市路径 / 违约瀑布违约方）；琥珀 `--warn` = 制度与门槛（法律基座 / 准入门槛·开户适当性 / 集合竞价竖线 / 停复牌回环 / 制度地缘执法泳道 / medium 置信）；绿 `--accent` = 通道与终局（接入链与外资通道 / DvP 终局 / ccp_novation / high 置信）；蓝 `--info` = 主体与交易层面（监管主体 / 投资者结构 / 午休与动态带〔蓝虚线〕/ 交易层面泳道）；金 `--sp-gold` = 交割管线存活会员层专属令牌；灰系 `--fg-faint` 虚线 / 虚线框 = 占位与未记录（四模块诚实三态共用同一线型语义）。
- **线型**：实线 = 已证机制；虚线 = 条件性 / 带（动态带 5 4、波动走廊 2 2、熔断长虚线 8 3——三者仅剖面使用，画布其余 6 模块无冲突复用）；`4 3` 短虚线跨模块统一表「占位 / 未记录」。
- **发现并修复的令牌缺口**：`--sp-gold` 只在 `:root[data-theme="dark"]` 覆盖、漏了 `prefers-color-scheme` 系统暗色分支——系统暗色用户交割管线金色层错用浅色值。已补齐。

**已知视觉遗留处置（§一 item 2 逐条）：**

- **收口**：成本瀑布「全零市场合计 0.00 bp」——合计行区分真零与未知零（该侧有 `type:none` 且无未结构化 / 幽灵条 → 写 0.00 bp；否则画「— 未摘引到可折算费率」不画条，副标题同口径、一侧未摘全时标「合计为下限」）；成本瀑布暗色「此侧不征」与侧未激活虚线 `--border-strong` → `--fg-muted`；交割管线无 bearer 预防层 opacity 0.22 → 0.32（暗色可读）；风险旗标 low 卡左缘色条 0.3 → 0.45（现网 risks 章已无 low 卡，[ADR-079]/[ADR-094] 收口成果，此项转为对将来数据的加固）；上市生命周期中英混排——[ADR-093] 已关闭，本棒 headless 复验无回归。
- **判定为诚实呈现、不改几何（记录理由即关闭）**：T+1 现货所右半留白（`Nmax = max(settleDays, 2)` 是 [ADR-051] 已拍板的折中，T+1 后本来就没有现金侧动作）；单一费种远大于其余时的镜像留白（买卖两侧同尺度对比是本图的核心语义，分侧缩放会失真）；按股 / 定额费折算较粗（≈ 标记 + 假设口径已在图例写明，精细化需数据层逐所假设）。
- **结构性遗留、维持原触发条件**：违约瀑布 `resource` 短语英文态仍中文（[ADR-051] 触发条件不变：`resource` 放宽为 `{zh,en}` + 数据窗口回填约 70 短语）；模块说明段 `[ADR-xxx]` 裸链接（[ADR-072]「未做」段，相关但独立）。

**巡检与非专业读者实测：**

- **headless 巡检**：Playwright（chromium headless shell）20 家 × 中英 × 明暗 = 80 组合画布截图 + DOM 断言（7 section 齐全、每 section 非 thin、有 SVG）+ console error 零捕获，80/80 通过；「更多」四视图（matrix / timezone / health / exchange）× 2 形态冒烟通过。修复后全量复跑仍 80/80。
- **非专业读者实测（用户指令授权由 agent 执行）**：sg-sgx 中文态整页通读（1440×6954，7 section）。判定**通过**——七屏依次回答「谁在管 / 谁在场上怎么进场 / 盘中怎么走 / 一笔交易花多少 / 成交后 T+N / 证券一生 / 要当心什么」，图例常驻、诚实三态一致，滚完可复述：MAS 监管、资金自由进出、机构主导、无涨跌停但有 ±10% 动态带与熔断、显性成本约 2.7 bp 往返、T+2 交收、CCP 六层违约瀑布、上市 8–10 周、主要当心流动性与制度变更。通读中核查的「参与者图头部文字叠压」为元素截图伪影（整页截图无叠压）。
...
5. **`cn-szse` 整理期 15 交易日：中文规则可取（修正同会话早前「全域不可达」的过头记录），但数字关卡未破。** 实测 szse.cn 分子域可达性分裂：www/english 主站 TCP 层不通（海外 IP 被拒），而 `docs.static.szse.cn`（规则 PDF 静态 CDN）与 `investor.szse.cn`（投资者教育子站）200 可达——中文 2026 修订全文已取（9.6.2「退市整理期的交易期限为十五个交易日」，字段 quote 即出此文）、创业板官方英译 2022 版已取（10.7.2『The delisting period includes fifteen trading days』）：**中英文一手均为拼写数字**，5b 的阿拉伯数字硬关卡仍过不去。同份英译对终止上市复核期混用阿拉伯『15-trading day period』（语义是复核期、不可挪用）——证明 SZSE 译文数字写法不统一，后续找阿拉伯锚点必须逐条核对语义。仅主板英译 2024 版未核（挂 www 主站、wayback 无快照），待人工下载。
