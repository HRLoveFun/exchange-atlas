...
**编号说明（分支背景，非设计内容）：** 本条设计最初在并行的 `worktree-v1-wave1` 分支上被定为该分支的「ADR-018」；但本分支（`worktree-agent-a43b5f89e21564f69`，从 `main` 的 PR#2 `c259a42` 分出）在两分支于 `9d39dc9` 分叉之后，独立追加了另一条不同内容的 ADR-018（`review_system` 矩阵列枚举覆盖率问题，见上条）。两条 ADR-018 编号相同但内容不同，是分叉开发的产物，不是同一决定的重复记录。为避免本分支内编号冲突，把移植过来的这条设计记为 ADR-019；orchestrator 后续合并两分支时，两边的编号需要人工对齐（例如把某一分支的相关 ADR 整体重排号），届时请一并处理，本条不代为决定重排方案。

**背景：** 用户实测发现两处系统性缺口，不是个别交易所漏填，而是 schema 本身没预留位置：
1. **市场结构与产品体系脱节**：`schema/taxonomy.yml` 第五章 `market_structure` 是单一扁平结构，隐含"一个实体只有一类产品的交易机制"的假设。但港交所（本分支正在处理的样本）第四章「产品体系」此前完全没列期货/期权类产品，第五章更是从未覆盖衍生品自己的交易时段/撮合机制/价格限制/熔断规则——不是"查不到"，是这些字段从未被要求覆盖现货以外的产品线。这与 `de-eurex`（纯衍生品所，`listing` 整章不适用，已有先例）是同一根问题的更严重表现——那处是"字段取值被迫近似/整章空置"，这里是"整整一条产品线的机制信息从未被建模"。
2. **指数体系的"交易所"与"市场"两种颗粒度混淆**：港交所只收了恒生指数，未收同一家公司（恒生指数有限公司）编制、成分股同样来自港交所上市证券的国企指数与恒生科技指数——这是纯粹的数据缺口，不需要改 schema，本条 ADR 只是顺带借这次改动把 `scope` 字段补上，为将来记录成分股天然跨交易所的指数（如标普500）预留位置。

**定了什么：**
1. **`market_structure` 增设 `derivatives`（`kind: object`）子字段**，字段集合以第五章顶层字段集合为基础裁剪：保留 `trading_sessions`/`opening_mechanism`/`closing_mechanism`/`matching_principle`/`order_types`/`tick_size`/`price_limits`/`circuit_breaker`/`volatility_interruption`/`trading_halt_mechanism`/`block_trade`/`market_maker_scheme`/`connect_schemes`；去掉现货语境特有、对标准化合约类衍生品普遍不适用的 `board_lot_size`/`odd_lot_handling`/`intraday_reversal`/`short_selling`/`dark_pool`（沿用"如实留空 + detail 说明设计前提不适用"惯例，但既然这五个字段对衍生品场景几乎必然不适用，直接不放进子字段结构，比让每个填写者对同五个字段重复写"不适用"更干净）；新增两个衍生品特有字段 `contract_specs_note`（合约规格摘要，如合约乘数/到期月份序列）与 `margin_practice_note`（交易端保证金制度摘要，与 `clearing.initial_margin_practice`——清算端保证金方法论——视角不同、不合并）。语义：顶层字段继续默认表达"现货/主板市场"的机制（对纯现货交易所无感知、无需改动），`derivatives` 只在该实体自身运营重要衍生品业务线时才填。不新增第十二个章节——挂在现有第五章下，按 `sync.py`/`app.js` 已经支持的"嵌套 object 递归渲染"（`trading_sessions`/`price_limits` 已是先例）直接工作，不用改任何 Python/JS 代码，是纯 schema+数据层面的改动。`derivatives` 子字段不挂 `in_matrix`——样本量不足时的空列会把矩阵稀释成"一眼看不出是没有还是没填"，这类深度信息留在档案页，不上矩阵。
2. **`indices` 列表项 `item_schema` 增设 `scope`（`enum_ref: index_scope`，值 `exchange` 交易所自身指数 / `market` 跨交易所市场基准）**。`exchange` 是既有惯例的显式化（现有条目默认补 `exchange`）；`market` 用于标普500/罗素2000/道指这类成分股不专属于单一交易所的基准指数，本条 ADR 只新增字段定义，不在本分支内批量回填其他交易所的 `market` 型指数。

**为什么不做集中式指数登记表：** 考虑过，否决。集中登记表能彻底解决"细节只在一处"，但代价是新增一个独立的数据文件 + `sync.py` 合并步骤，为个位数几个近乎不变的静态事实新增一条永久性构建流水线，成本与收益不成比例。
