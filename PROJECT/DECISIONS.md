...
**同日四轮迭代增补（用户复看反馈一点）：** 右侧导航收起态由「一列细横线」改为**一个半透明圆形按钮**（34px 圆、`color-mix` 半透明底 + 背景模糊，更不显眼）；hover 按钮区域切换为展开的导航列表（模块名 + 细横线，当前模块高亮不变），列表右侧与圆钮同缘、指针不离开容器即保持展开。其余（点击跳转 / `replaceState` 同步深链 / 仅画布视图显示 / 窄屏隐藏）不变。**四轮验证：** `make build` 全绿、`node --check` 通过；headless 实测收起态圆钮、hover 展开列表、展开态点击跳转（落点交割管线、active 跟随）全过。
