...
1. **风险旗标 · 数据子棒**（[ADR-066] 分棒 ①）— **已估清，28 字段，待执行**（[ADR-079] 补记：用户 2026-09-05 拍板范围取 28、走第二人独立复核、定为 Phase 4 硬前置）。构成（按 09-05 现算重排）：`fx_risk_note` 20 家全扫（17 家 `low` 就地清 + 3 家 `medium` 复核，其中 `fr-euronext` 补字段级 `sources`）+ 补 3 处空 `political_risk_note`（`cn-sse`/`hk-hkex`/`tw-twse`）+ `enforcement_note` low 簇 3 处（`cn-sse`/`cn-szse` 空、`kr-krx` `low`）+ `ch-six`/`cn-szse` 的 `political_risk_note` `low` 就地清。**逐所落地清单（20 家 × 货币当局 / 域名 / 登记状态 / 候选入口 / 预期终态）见 [ADR-079] 补记第四节**——18 个候选域名（17 家货币当局 + `imf.org`）已登记仅 3 个（`mas.gov.sg` / `rbi.org.in` / `resbank.co.za`），登记域名是每家闭环的第一步。与 [ADR-060] 任务四并轨。风险旗标渲染层已落地（[ADR-066] 分棒 ②，2026-09-04）。**执行中（2026-09-05）：15/28 字段完成、第 12 章不变式已实装（validate 校验 20），进度与剩余清单 / 各所取数坑见 §三。**
...
4. **数据空缺复核轨剩余**（横切，与上并行；[ADR-060]/[ADR-062]）— ~~任务三~~（已完成 2026-09-05，[ADR-086]）；**任务四**（5 家旗舰所深度 F 桶 78 处，`us-nyse`/`hk-hkex`/`uk-lse`/`cn-sse`/`jp-jpx`，执行方案已定案 [ADR-078]，建议 Phase 4 前完成、非硬前置）**78/78 全部回填完成**（2026-09-05 交互式会话收尾：剩 11 处逐处自检收口，`OPEN-QUESTIONS.md` 第29条已删除，残存微缺口见三节）。任务二第二人独立复核已完成（[ADR-074]，2026-09-05，79 处终态 96.2% 达标）；任务五 ①②③ 全部完成（①② [ADR-075]，③ stale 复核清单 [ADR-088]，2026-09-05），`kr-krx` 剩 8 处 low 待人工投喂。**新增独立棒（用户 2026-09-05 拍板立项，[ADR-079] 补记 / `OPEN-QUESTIONS.md` 第 28 条）：`volatility: stable` 且 `confidence: medium` 却零 `sources` 的全库 64 处**——头部为 `overview.timezone` 16 / `settlement_currency` 16 / `dst_rule` 14 / `trading_currency` 5 / `regulation.clearing_regulator` 5；不从风险旗标数据子棒扩张（该子棒只在第 12 章加限定不变式），独立排期，与上面③同批或先于③做均可。另有已收口的成本瀑布残差（[ADR-065]）/ 长尾（[ADR-067]）遗留移交项：`kr-krx exchange_fees` 当期档位（KRX 数据端点对数据中心 IP 封锁，[ADR-075] 已探明，仍需人工投喂）、`us` Section 31 FY2027 公告（未发布，OPEN-QUESTIONS #88）、`fr-euronext stamp_duty`（一所多国、`rate: null` 是正确终态）。详版见三节。

完整清单与每章的小型 `spec` 补充见三节 `- [ ] **Phase 3 · 其余章节可视化**`。

### 最近完成（滚动窗口，只留最近 3 条；更早的见三节）

- **2026-09-05 · 任务五③ `make check` stale 复核清单上线**（[ADR-088]）— `validate.py` 校验项 19：超阈值（按超期天数降序）与未记 verified（无法判定）分两组、warning 不阻断，freshness_rows 由 `validate_generated_blocks` 顺路复用；selfcheck 78→87 锁行为（全库现算 stale=0）；[ADR-060] 评估题「是否固化进 CLAUDE.md §八」结论=不固化，宪法零改动。
- **2026-09-05 · 任务三五棒（C 桶 40 处）执行落地 + 独立盲审**（[ADR-086]）— 18 填实 + 2 not_applicable + 20 留空（OQ 第30/31条），盲审初检 88.9%、订正后 18/18 PASS、零幻觉。
- **2026-09-05 · adr-heal.yml 改走开分支+PR+auto-merge**（[ADR-087]）— 不再直推 main，修复 branch protection 拦截 bot 直推导致 main 两度变红的问题。
...
- [ ] **Phase 3 第七棒分棒① · 风险旗标数据子棒**（方案定案 2026-09-05，[ADR-079]；**执行中，15/28 字段完成**，2026-09-05）— [ADR-066] 轴 6 留的独立数据层子棒，与 [ADR-060] 任务二 / 四并轨。渲染层已把 `confidence` 做成该章面板一等视觉信号（[ADR-066] 轴 3），`fx_risk_note` 近全库 `low` 使 20 家「交易层面」泳道恒显空心旗标、读不出跨市场差异——子棒目标是**让这一格在图上重新携带信息**，不是把 low 改成 medium 这个动作本身。
    - **执行进度（2026-09-05，交互式会话，每所一个 commit）** —— ① **第 12 章不变式已实装**（validate 校验 20：risks 5 `*_note` 字段 `confidence: medium|high` 必须有**字段级** `sources`、raw 口径不认 `_meta` 继承；selfcheck 93 探针全过），首批命中 2 处（`fr-euronext.fx_risk_note` / `de-eurex.liquidity_risk_note`）已随批次修复。② **已完成 15 字段 / 9 家**：`in-nse` fx（RBI 行长声明 quote，low→high）/ `sg-sgx` fx（MAS 框架 FAQ quote 经 wayback，low→high）/ `za-jse` fx（SARB 一手无制度原文→IMF Article IV quote，medium）/ `hk-hkex` fx（HKMA LERS 双页 quote，low→high）+ political（基本法第 5 条，空→high）/ 欧元区三家 fx（ECB explainer「汇率非政策目标」quote：`de-eurex`/`de-xetra` high、`fr-euronext` medium——NOK 部分 Norges Bank 原文缺）+ `de-eurex` liquidity（Börsenordnung §49 做市报价义务为制度锚）/ `jp-jpx` fx（BOJ 官方说明 quote，low→high）/ `cn-sse`+`cn-szse` fx（《外汇管理条例》27 条 + 人行三点表述 quote，low→high）/ `ca-tsx` fx（BoC explainer quote，low→high）/ `us-nyse`+`us-nasdaq` fx（NY Fed FX 季报 quote，low→medium）。③ **剩余 13 字段**：`kr-krx` fx+enforcement、`tw-twse` fx+political、`cn-sse`+`cn-szse` political+enforcement（5 处 C/D/E 组）、`au-asx`/`br-b3`/`ch-six` fx+political、`sa-tadawul`/`uk-lse` fx。④ **已知待办/坑**：SARB `/content/dam/` PDF 被 WAF 拦（Request Rejected）；MAS 主站全站 Maintenance 壳（绕过 looks_blocked，勿全所盲刷）；nseindia 对 curl 反爬复活（22KB 壳页覆盖好缓存，已删坏缓存退 CACHE_MISS）；`norges-bank.no`/`bankofengland.co.uk`/`rba.gov.au`/`bcb.gov.br`/`snb.ch`/`sama.gov.sa`/`bok.or.kr`/`cbc.gov.tw`/`federalreserve.gov` 未登记（按需登记，先域名+栏目）；`za-jse`/`us-nyse`/`us-nasdaq` 升 high 需 IMF AREAER de jure 原文（elibrary 403，wayback 可达但编号待定位）。
...
- [x] **架构优化 A/B/C：数据加载去重 / DECISIONS 归档阈值 / 前端共享层收敛**（横切条目，2026-09-05，[ADR-083]/[ADR-084]/[ADR-085]）— ① `tools/data_files.py` 收敛 4 处 `data/exchanges/*.yml` 遍历样板，`make build` 全绿零 diff（A）；② DECISIONS.md 归档阈值定案（超 3500 行触发），`validate.py` 加非阻断 warn 提醒拆 `DECISIONS-ARCHIVE.md`（B）；③ `app.js` 七模块 clone-and-own 收敛为 6 个共享函数，新增 `check_no_dup_render_helpers.py`（严格字节比对、零假阳性）挂 `make check`，Playwright 308 个 `(view, exchange, lang)` 组合回归零 diff（C）。
...
- [x] **任务三 · 9 家衍生品子章残余补全（C 桶 40 处）**（穿插 viz 模块之间；执行方案见 [ADR-082]，2026-09-05 实算侦察后按「上次卡在哪」重排为五棒；**2026-09-05 五棒执行落地 + 独立复核订正，见 [ADR-086]**）
  - **结果**：终态 = 填实 18（棒 1 六处 clearing.derivatives 全 high；棒 2 六处 connect/closing/after_market/block_trade；棒 3 br-b3 price_limits 两处按 IND/WIN 代表合约；棒 4 holidays_note 四处——au/fr/hk/br）+ 字段级 `not_applicable` 2（sa-tadawul/in-nse `other_boards`，裁定②合格样本）+ 留空 20（`detail` 均写明判据，汇总于 `OPEN-QUESTIONS.md` 第 30/31 条；sg-sgx 9 处降级待人工投喂合约规格 PDF）。棒 0 前置：schema/spec.yml 衍生品 14 形状（anchor 复用）+ 两侧同源不变式机器化，selfcheck 68→73。
  - **关键发现**：hk-hkex 衍生品有**专属假日安排**（HKFE 年度公告）且 MSCI/货币合约在港公众假期照常交易——与证券市场的实质差异；cn-szse 的深港通**不含**衍生品由办法第十六条穷举标的条款正面支撑。方法论沉淀：负面断言正面化 = 找**穷举性原文**（穷举标的条款/穷举时段表/年度假日日历），推断「无 X」判 medium 不判 high。
  - **验收**：`make build` 全绿、verify_quotes FAIL=0；独立视角复核（全新 agent 盲审 20 处 × 7 维度）初检 88.9%，2 FIX 就地订正 + 2 QUESTION 经补 .txt 伴随整段核对转 PASS，终态 18/18 全 PASS、零幻觉。进度矩阵变动：`hk-hkex`/`sa-tadawul` 第 8 章 🟡→✅。
...
- [x] **任务四 · 5 家旗舰所深度补全（F 桶逐所 78 处，执行方案已定案，[ADR-078]）**（建议 Phase 4 合并启动前完成，非硬前置）
  ...
  - **✅ 第二轮整批第二人独立复核（2026-09-05，用户指定加验）**：第一轮复核后新终态的 13 处（收尾回填 11 处 + uk 2 处再更新）未经第二人复核，虽 [ADR-080] 裁定「回填后逐处自检即可」，用户指定仍整批加验——**78 处终态全量重核**（机器反查/来源工程全量脚本化 + 13 处语义深读 + 漂移审计 `git diff 1cd06d3..HEAD` 限定 78 字段）。初检 **76 PASS / 2 FIX（均 detail 级：us foreign_access_channel 补 FDI/组合投资区分依据、uk major_outage_history 补 2019 具体到日为媒体口径）/ 0 未决 QUESTION**（3 项 Q 就地证伪：uk pre_market kind 结构性读表、cn 沪股通归属经 2024 卷同表证实、hk 两处推断已声明）；机器层零问题（quote 逐源命中、wayback 4 处沿用 [ADR-075] 口径、域名登记全过）；jp-jpx 零漂移。**78/78 filled 全部 PASS，零幻觉维持，任务四验收关卡关闭**。判定表见 `PROJECT/DATA-GAP-TASK4-SPOT-CHECK.md` 第二轮章节。
...
- [x] **任务五 · 抓取基础设施修复**（2026-09-05 全部完成：①② [ADR-075]，③ [ADR-088]）
