...
1. **成本瀑布数据层残差（A2/A3 未竟项，按触发时点推进）** — `kr-krx exchange_fees` 需抓 KRX 当期收费表确认临时阶梯到期后的现行费率；`us-nyse`/`us-nasdaq` SEC Section 31 费率 2026-10-01 起按 SEC FY2027 公告重核；`ca-tsx regulatory_fees` / `kr-krx regulatory_fees`（「无按笔监管费」仍缺干净一手源）；`uk-lse stamp_duty` 的 `side: buy`（需 HMRC 非 SPA 端点或 legislation.gov.uk 条文）。明细见 OPEN-QUESTIONS #88 与 `PROJECT/COST-WATERFALL-SPOT-CHECK.md`
...
4. **其后按交易员价值继续**：上市生命周期（一并落地 [ADR-036] #5 章节级"仅现货适用"标记）→ 监管图 → 参与者 → 风险旗标（B 组 `fx_risk_note`/`kr-krx` low 簇就地清）——每个模块的设计定案按 [ADR-057] 的 merge-ready 清单逐条回答
...
- **2026-09-01 · 北极星定案 · 单页画布合并**（[ADR-057]）— 可视化模块终态合并为单页画布、其余视图降「更多」入口，立 merge-ready 设计清单约束 Phase 3 剩余模块；合并本身新增为 Phase 4，承接 [ADR-056]「遗留」
- **2026-09-01 · 剖面视觉迭代 · 机制核心面板 + 透视开关**（[ADR-055]）— 「交易机制」七项从主图下方收进主图中心一块固定 628×276、垂直居中于零轴的 `foreignObject` 面板；面板右上角 `◐/●` 透视开关露出被盖的零轴/熔断线/走廊；纯前端两文件、`docs/data/` 零 diff
...
- [x] **Phase 3 第二棒（数据层收尾）· 成本瀑布 13 降级点 + 6 补强点一手源坐实 + `validate.py` 5c 机器化**（2026-09-01/02 完成，5c 见 [ADR-058]；数据层部分承接 [ADR-054] 留下的坐实项）— 分三棒：
  - **A1 · 5c 机器化**（[ADR-058]）— `validate.py` 在 5b 外新增 **5c**：`spec.note` 字符串里的数字必须在同字段 `quote`/`zh`/`detail` 三处任一命中，把「note 夹带无出处数字」从人工抽检变成构建关卡；连带清现网 13 处残差（`cn-sse` `0.0341‰`、`br-b3` `0.00500%`/`0.00375%`、`fr-euronext`「法国现行 0.3%」等）。已做正向/负向验证：5c 能拦下 `0.0341`、正向放行合法 note。
  - **A2 · 13 个 `type: none` 降级点坐实** — 用 `make fetch` 重抓税法/税务局一手原文逐条复核。方法学结论：**`rate: null` 本身是审慎正确的终态**——一手税务局页通常只覆盖自身税种、**不证伪** FTT/监管费，能翻回 `type: none` 的仅限拿到「肯定性不征」一手陈述者。据一手源**改回 `type: none`** 2 个（`kr-krx stamp_duty` ← PwC 韩国：stamp tax 针对「制备证明财产权设立/转让/变更的文书者」、非证券转让；`hk-hkex financial_transaction_tax` ← IRD 印花税制度 + 港交所费率页：港股从价交易税负即印花税）；**源升级但仍维持 `rate: null`** 1 个（`au-asx` ← Baker McKenzie Australia "Stamp duty is not payable on share transfers"，未逐字出现 "no FTT" 故不硬翻）；缺干净一手源维持 `rate: null` 2 个（`ca-tsx`/`kr-krx regulatory_fees`）；余 8 点维持并在 OPEN-QUESTIONS #88 留痕。
  - **A3 · 6 个 `side`/费率补强点坐实** — 5 个落地：`kr-krx` STT `side: sell`（韩国《证券取引税法》英文版 elaw.klri.re.kr，transferor=让与人=卖方）、`za-jse` STT `side: buy`（SARS "applies to the purchase" + 可向受让人/买方追偿）、`us-nasdaq regulatory_fees`（eCFR 17 CFR 240.31 "covered sale" 定义）、`br-b3` IOF `side: buy`（B3 "incoming resources"）；1 个残差 `uk-lse` SDRT `side: buy`（gov.uk/HMRC 为 SPA，未取到逐字买方原文，字段保留 `side: buy` 待抓非 SPA 端点）。
  - **退出验收**：`make check` 全绿（`validate` 20 家 0/0、`verify_quotes` OK=1000 / FAIL=0 / CACHE_MISS=78、`check_ui_i18n` OK）、`make build` 全绿；`PROJECT/COST-WATERFALL-SPOT-CHECK.md` 逐家明细 8 行 + 「三类系统性缺口」第 2/3 条已回写 A2/A3 状态；`ROADMAP` 下一步只留时效类残差（KRX 当期收费表、SEC FY2027 重核、ca/kr 监管费、`uk-lse` side）。新增登记一手域名：`ecfr.gov`、`elaw.klri.re.kr`、`resourcehub.bakermckenzie.com`（au）、`taxsummaries.pwc.com`（kr）。
...
- [ ] **Phase 4 · 单页画布合并** — 把市场机制剖面 / 成本瀑布 / 交割管线 / Phase 3 剩余模块合并为同一页的一块可视化画布，对比矩阵 / 时区甘特条 / 数据健康度 / 档案页降级到「更多」入口（北极星定案见 [ADR-057]，此前只在 [ADR-056] 与宪法开篇点到）。Phase 3 剩余每个模块的设计定案按 [ADR-057] 的 merge-ready 清单逐条回答（锚定关系 / 占位 / 诚实三态 / 语言开关 / 零构建）。
  - **启动前置条件（硬顺序，不插队）：Phase 3 的全部章节可视化模块——上市生命周期 / 监管图 / 参与者 / 风险旗标——均已落地。** 在此之前不启动合并、不动前端做画布布局；单项没做齐就合并会反复重排（[ADR-057] 定了什么 #4，用户 2026-09-01 复述强调）。
