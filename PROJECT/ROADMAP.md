...
北极星（[ADR-057]）：这几个按主题分的视图是过渡形态，终态合并为单页可视化画布、其余视图（矩阵 / 时区 / 健康度 / 档案页）降级到「更多」入口。Phase 3 剩余模块按 [ADR-057] 的 merge-ready 清单设计，合并本身是 Phase 4。
...
5. **其后按交易员价值继续**：上市生命周期（一并落地 [ADR-036] #5 章节级"仅现货适用"标记）→ 监管图 → 参与者 → 风险旗标（B 组 `fx_risk_note`/`kr-krx` low 簇就地清）——每个模块的设计定案按 [ADR-057] 的 merge-ready 清单逐条回答
6. **Phase 3 模块做齐后 → Phase 4 单页画布合并**（[ADR-057]）：合并画布的整体布局形态 / 「更多」入口形态 / 各模块排序，启动时 Q&A 定
...
- [ ] **Phase 4 · 单页画布合并** — 把市场机制剖面 / 成本瀑布 / 交割管线 / Phase 3 剩余模块合并为同一页的一块可视化画布，对比矩阵 / 时区甘特条 / 数据健康度 / 档案页降级到「更多」入口（北极星定案见 [ADR-057]，此前只在 [ADR-056] 与宪法开篇点到）。**排在 Phase 3 全部模块做齐之后**——模块集合稳定前合并会反复重排布局。Phase 3 剩余每个模块的设计定案按 [ADR-057] 的 merge-ready 清单逐条回答（锚定关系 / 占位 / 诚实三态 / 语言开关 / 零构建）。
