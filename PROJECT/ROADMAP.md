...
- [ ] **Phase 3 第七棒分棒① · 风险旗标数据子棒**（方案定案 2026-09-05，[ADR-PENDING-risk-flags-data-plan]；执行未开始）— [ADR-066] 轴 6 留的独立数据层子棒，与 [ADR-060] 任务二 / 四并轨。渲染层已把 `confidence` 做成该章面板一等视觉信号（[ADR-066] 轴 3），`fx_risk_note` 近全库 `low` 使 20 家「交易层面」泳道恒显空心旗标、读不出跨市场差异——子棒目标是**让这一格在图上重新携带信息**，不是把 low 改成 medium 这个动作本身。
