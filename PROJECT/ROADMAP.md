...
4. **数据空缺复核轨剩余**（横切，与上并行；[ADR-060]/[ADR-062]）— **任务三**（9 家衍生品子章 C 桶 40 处，穿插 viz）；**任务四**（5 家旗舰所深度 F 桶 78 处，`us-nyse`/`hk-hkex`/`uk-lse`/`cn-sse`/`jp-jpx`，执行方案已定案 [ADR-078]，建议 Phase 4 前完成、非硬前置；族A 清算保证金四件套 20 处已回填，2026-09-05，剩 58 处按族D→B/C→E/F/G/H→I 顺序推进）。任务二第二人独立复核已完成（[ADR-074]，2026-09-05，79 处终态 96.2% 达标）；任务五 ①② 已完成（[ADR-075]，2026-09-05，`[OTP]` 抓取 + wayback 回退 + `za-jse` 缓存重建），③（`make check` stale 清单）未做、留独立排期，`kr-krx` 剩 8 处 low 待人工投喂。另有已收口的成本瀑布残差（[ADR-065]）/ 长尾（[ADR-067]）遗留移交项：`kr-krx exchange_fees` 当期档位（KRX 数据端点对数据中心 IP 封锁，[ADR-075] 已探明，仍需人工投喂）、`us` Section 31 FY2027 公告（未发布，OPEN-QUESTIONS #88）、`fr-euronext stamp_duty`（一所多国、`rate: null` 是正确终态）。详版见三节。
...
- [ ] **任务四 · 5 家旗舰所深度补全（F 桶逐所 78 处，执行方案已定案，[ADR-078]）**（建议 Phase 4 合并启动前完成，非硬前置）
  - **进展（2026-09-05）**：**族A · 清算保证金四件套 20 处已回填**（5 家 × `initial_margin_practice`/`maintenance_margin_practice`/`mark_to_market_frequency`/`last_trading_day_rule`，另有 `hk-hkex clearing.delivery_method` 由 low 升级 medium——该字段在 78 处清单内但属族A同章顺手坐实）。全部一手官方来源（govinfo 联邦公报/dtcclearning 组件指南/eCFR Reg T/FINRA 4210、HKEX 保证金页＋IMC 指南、中国结算两份办法 PDF＋上证发〔2026〕5号、LCH Ltd EquityClear Service 程序手册、JSCC 官方页），high 字段 quote 全部过 `verify_quotes` 离线反查（OK=1067/FAIL=0）；`us-nyse`/`cn-sse`/`uk-lse`/`jp-jpx` clearing 章进度矩阵 🟡→✅（`hk-hkex` 仍 🟡，其 clearing 章另有族A外字段待后续族）。检索按族批量、落盘按所归拢 5 个 commit（+1 构建产物 commit），每所一个 commit 的抽检自检在交付时同步做，**族A 不触发第二人复核**（20 处 < 30 字段门槛；78 处总量门槛仍在全部完工后统算）。**已知工具坑（已记 `PROJECT/sources/jp-jpx.md`）**：`tools/fetch.py` 的 `looks_blocked` 会对 jpx.co.jp 页面（HTML 内嵌 cloudflare 字样）误报 FAIL，用 `fetch_sources.py --ex jp-jpx`（requests）即可全过。
  - **下一步（族D 起）**：税费/佣金约 15 处（复用 [ADR-065]/[ADR-067] 判据）→ 族B/C listing 13 处 → 族E/F/G/H 约 14 处 → 族I risks 残余 3 处待「下一步 1」完工。
