...
- [ ] **Phase 3 · 其余章节可视化** — 成本瀑布 → 交割管线 → 上市生命周期（一并落地 [ADR-036] #5 的章节级"仅现货适用"标记）→ 监管图 → 参与者 → 风险旗标（B 组 `fx_risk_note`/`kr-krx` low 簇就地清）→ …按交易员价值迭代，每章带一次小型 `spec` 补充。**并入 [ADR-042] 的第五章三字段 + 四个 spec 形状的 20 家回填**（`execution_model` 达 ≥16/20 后补 `in_matrix: trading_mechanism`）。
  ...
  - [ ] **Phase 3 第三棒（渲染层）· `renderSettlementPipeline`** — 英文版可用性修订已合并（[ADR-049]），`app.js`/`index.html`/`styles.css` 合并冲突已解除，可开工。清单：`docs/assets/app.js` 双泳道 SVG + 违约瀑布常驻附图（按 `default_management.spec.layers[].bearer` 上色）+ 据 `guarantee_model` 选「CCP 介入」节点图形 / `index.html` 顶层 tab + 路由键 `settlement-pipeline` / `styles.css` `.sp-*` / Chrome headless 截图核对。**新代码从一开始接语言开关**（吸取 [ADR-047] 渲染层未接、留给英文版修订补的教训）。见 [ADR-048] 三方向。
