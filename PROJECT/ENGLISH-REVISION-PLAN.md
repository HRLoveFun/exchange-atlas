...
> 状态：**已审查，待排期执行**。本文件是一次性的审查报告 + 实施计划，延续 [ADR-024]/[ADR-026] 的英文版审查线。
> 执行完成后：按批次在 ROADMAP 打勾、写一条 ADR（暂定 ADR-048），本文件转「已归档」保留作记录。
...
- 全部完成后 `PROJECT/DECISIONS.md` 写一条 ADR（暂定 ADR-048），至少记：
