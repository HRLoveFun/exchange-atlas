...
- **任何批量数据 / `spec` 工作，交付前抽检值与原始出处，通过率 < 95% 就停下来修流程，不带着已知错误率继续铺开。** 抽检者必须是**独立视角**——可以是人，也可以是另一个未共享上下文的全新 agent 会话（不是 fork：fork 继承同一上下文的信念，原始抓取错了大概率也查不出来），协调者/执行者本人自查不算数（[ADR-PENDING-solo-automation-pipeline]，2026-09-05 起措辞由"人工抽检"改为"独立视角复核"，实践先例见 [ADR-074]）。阈值不随样本量放宽（[ADR-017]）；某家 / 某批不过只暂停复核该家 / 该批，不牵连已过关的。历史实例：v0.1 上交所抽检 20 字段、v1.0 每新增一家抽检 10 字段，都按此执行。
