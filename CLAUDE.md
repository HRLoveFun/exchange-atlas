...
- v1.0 阶段性验收（按所评估，不是按批次）：每新增一家交易所人工抽检 10 个字段核对 `quote` 与原始出处，**通过率仍需 ≥95%**——样本量比 v0.1 小是因为流程本身的可靠性已经被 v0.1/v0.2 证明过，但"这一家具体研究错了"是另一种风险，跟"流程整体行不行"是两回事，阈值不因此放宽，见 `PROJECT/DECISIONS.md` [ADR-017]。某家未过阈值只暂停复核该家，不影响同批次其他已过关的交易所。
