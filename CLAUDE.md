...
| 发生了什么 | 就去写 | 写成什么样 |
|---|---|---|
| 阶段/波次/某交易所批次跑完，或验收（抽检）出了结果 | `PROJECT/ROADMAP.md` 对应条目打勾 **+ 「一、当前状态」两处同步**（见下方「ROADMAP 回写要动两处」） | 带日期，写清楚结果（几/几家、抽检通过率），别只写"完成" |
...
**ROADMAP 回写要动两处**（文件结构见 `PROJECT/ROADMAP.md` 文首导航）：

1. **详版条目**（活跃阶段的计划节，当前是第三节「v2.0 计划」）——事实只在这里写一遍：日期 + ADR 编号 + 可核验结果（几家、通过率、零 diff、已知局限）。
2. **「一、当前状态」的两个子节**——只做速览索引，**不重复写结果**（写两遍违反第一节）：
   - **下一步**：条目完成后划掉，并从详版条目的「已知局限」里取新迭代点补入；空着等于下个会话不知道该干什么。
   - **最近完成**：滚动窗口只留最近 3 条，每条一行 + ADR 编号，新条目进来时移出最旧一条（细节已在详版条目，不丢）。
